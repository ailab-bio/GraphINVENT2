...
class GGNN(gnn.summation_mpnn.SummationMPNN):
    ...
    def __init__(self, constants: namedtuple) -> None:
        ...
        # An extra MLP (index n_edge_features) handles the virtual edge type used
        # during conditional generation; it is unused for unconditional models.
        self.msg_nns = torch.nn.ModuleList(
            ...
            for _ in range(self.constants.n_edge_features + 1)
        )

        # Condition encoder: maps property vector → seed-node embedding.
        # Only created when condition_dim > 0.
        self.condition_encoder: Optional[ConditionEncoder] = None
        if getattr(self.constants, "condition_dim", 0) > 0:
            self.condition_encoder = ConditionEncoder(
                condition_dim=self.constants.condition_dim,
                hidden_dim=self.constants.condition_embedding_dim,
                condition_embedding_dim=self.constants.condition_embedding_dim,
            )
    ...
    def message_terms(
        ...
        """
        # Use the actual number of edge channels present (may be n_edge_features
        # or n_edge_features + 1 when a virtual seed node is active).
        n_channels = edges.shape[-1]
        edges_v = edges.view(-1, n_channels, 1)
    ...
    def forward(
        ...
        """
        Runs message passing with optional property conditioning.

        When ``condition_vector`` is provided and a ``ConditionEncoder`` has been
        built, the vector is encoded into a seed-node embedding that is injected
        into every message-passing round via a virtual seed node.

        Args:
            nodes:            Shape (batch, max_n_nodes, n_node_features).
            edges:            Shape (batch, max_n_nodes, max_n_nodes, n_edge_features).
            condition_vector: Optional property values.
