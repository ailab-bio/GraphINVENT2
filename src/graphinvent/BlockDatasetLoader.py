...
class HDFDataset(torch.utils.data.Dataset):
    ...
    "edges", and "action_probs".  When a "condition_vector" dataset is also
    present (conditional training), a fourth tensor is yielded.
    """
    ...
    def __init__(self, path: str) -> None:
        ...
        self.action_probs = hdf_file.get("action_probs") or hdf_file.get("APDs")

        # Optional conditioning dataset (None for unconditional models).
        self.condition_vector = hdf_file.get("condition_vector")
    ...
    def __getitem__(self, idx: int) -> Tuple:
        ...
        if self.condition_vector is not None:
            cond_i = torch.from_numpy(self.condition_vector[idx]).type(torch.float32)
            return (nodes_i, edges_i, action_probs_i, cond_i)
