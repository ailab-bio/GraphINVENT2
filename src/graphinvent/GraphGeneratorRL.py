...
class GraphGeneratorRL:
    ...
    queried.  The agent's action probabilities is used to sample the next action; both the agent
    ...
    def __init__(
        ...
        batch_size: int,
        condition_vector: torch.Tensor = None,
    ) -> None:
        """
        Args:
        ----
            ...
            condition_vector (torch.Tensor|None) : Optional property condition
                repeated across the batch.  Shape (1, condition_dim) or
        ...
        self.model = model
        if condition_vector is not None and condition_vector.dim() == 1:
    ...
    def sample(
        ...
        generated_agent_loglikelihoods_sum = torch.sum(
            self.generated_agent_loglikelihoods, dim=1
        )[: self.batch_size]
        generated_prior_loglikelihoods_sum = torch.sum(
            self.generated_prior_loglikelihoods, dim=1
        )[: self.batch_size]
    ...
    def build_graphs(self) -> int:
        ...
        self.agent_likelihoods.zero_()
        self.generated_agent_loglikelihoods.zero_()
        self.prior_likelihoods.zero_()
        self.generated_prior_loglikelihoods.zero_()
        ...
        while n_generated_so_far < self.batch_size:
            ...
            if self.condition_vector is not None:
                cond = self.condition_vector.expand(self.nodes.shape[0], -1)
                agent_action_probs = softmax(
                    self.agent_model(self.nodes, self.edges, condition_vector=cond)
                )
                prior_action_probs = softmax(
                    self.prior_model(self.nodes, self.edges, condition_vector=cond)
                )
            else:
                agent_action_probs = softmax(self.agent_model(self.nodes, self.edges))
                prior_action_probs = softmax(self.prior_model(self.nodes, self.edges))

            # sample the actions from the predicted action probabilities
            add, conn, term, invalid, agent_likelihoods, prior_likelihoods = (
                self.get_actions(
                    agent_action_probs=agent_action_probs,
    ...
    def allocate_graph_tensors(self) -> None:
        ...
        self.agent_likelihoods = torch.zeros(likelihoods_shape, device=constants.device)
        self.prior_likelihoods = torch.zeros(likelihoods_shape, device=constants.device)
    ...
    def apply_actions(
        ...
        prior_likelihoods_sampled: torch.Tensor,
    ) -> None:
        """
        ...
        def _add_nodes(
            ...
            self.agent_likelihoods[batch, self.action_counter[batch]] = (
                agent_likelihoods_sampled[batch]
            )
            self.prior_likelihoods[batch, self.action_counter[batch]] = (
                prior_likelihoods_sampled[batch]
            )
        ...
        _add_nodes(
            add, generation_round, agent_likelihoods_sampled, prior_likelihoods_sampled
        )

        # then applies the "connect" action to all graphs in batch (note: does
        # nothing if a graph did not sample "connect")
        _conn_nodes(
            conn, generation_round, agent_likelihoods_sampled, prior_likelihoods_sampled
        )
    ...
    def copy_terminated_graphs(
        ...
        prior_likelihoods: torch.Tensor,
    ) -> int:
        """
        ...
        self.agent_likelihoods[terminate_idc, self.action_counter[terminate_idc]] = (
            agent_likelihoods[terminate_idc]
        )
        self.prior_likelihoods[terminate_idc, self.action_counter[terminate_idc]] = (
            prior_likelihoods[terminate_idc]
        )
        ...
        # Padding columns are exactly 0; map them to 1 so log() gives 0 and they
        # contribute nothing.  Slicing by the global round instead (as this used
        # to) clamped a restarted slot's leading zeros to 1e-6, adding a spurious
        # -13.8 per padded column to every reported log-likelihood.
        def _safe_log(x: torch.Tensor) -> torch.Tensor:
            return torch.log(torch.where(x == 0, torch.ones_like(x), x.clamp(min=1e-6)))

        self.generated_agent_loglikelihoods[begin_idx:end_idx] = _safe_log(
            generated_agent_likelihoods_local
        )
        self.generated_prior_loglikelihoods[begin_idx:end_idx] = _safe_log(
            generated_prior_likelihoods_local
        )
    ...
    def reset_graphs(self, idc: torch.Tensor) -> None:
        ...
        if len(idc) > 0:
            ...
            self.agent_likelihoods[idc] = torch.zeros(
                (len(idc), *likelihoods_shape[1:]),
                dtype=torch.float32,
                device=constants.device,
            )
            self.prior_likelihoods[idc] = torch.zeros(
                (len(idc), *likelihoods_shape[1:]),
                dtype=torch.float32,
                device=constants.device,
            )
    ...
    def get_actions(
        self, agent_action_probs: torch.Tensor, prior_action_probs: torch.Tensor
    ) -> Tuple[torch.Tensor, ...]:
        """
        ...
        Args:
        ----
            agent_action_probs (torch.Tensor) : action probabilities for a batch of graphs (agent).
            prior_action_probs (torch.Tensor) : action probabilities for a batch of graphs (prior).
        ...
        def _reshape_action_probs(
            ...
            -------
                ...
                f_conn (torch.Tensor) : Reshaped action probabilities segment for "connect" action.
                f_term (torch.Tensor) : Reshaped action probabilities segment for "terminate" action.
            """
            ...
            # get length of flattened segment of action probabilities corresponding to "add"
            # action
        ...
        def _sample_action_probs(
            agent_action_probs: torch.Tensor,
            prior_action_probs: torch.Tensor,
            batch_size: int,
        ) -> Tuple[torch.Tensor, ...]:
            """
            ...
            -------
                ...
                prior_likelihoods (torch.Tensor) : Contains prior likelihoods
                                                   for sampled actions.
            """
            action_dist = torch.distributions.Multinomial(1, probs=agent_action_probs)
            action_probs_one_hot = action_dist.sample()
            f_add, f_conn, f_term = _reshape_action_probs(
                action_probs_one_hot, batch_size
            )

            agent_likelihoods = agent_action_probs[action_probs_one_hot == 1]
            prior_likelihoods = prior_action_probs[action_probs_one_hot == 1]
        ...
        f_add_idc, f_conn_idc, f_term_idc, agent_likelihoods, prior_likelihoods = (
            _sample_action_probs(
                agent_action_probs=agent_action_probs,
                prior_action_probs=prior_action_probs,
